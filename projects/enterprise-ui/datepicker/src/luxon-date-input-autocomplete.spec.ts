...
describe('LuxonDateInputAutocomplete', () => {
  ...
  it('supports year-first formats and compact input', () => {
    const result = new LuxonDateInputAutocomplete('yyyy-MM-dd').process('202603', { now });

    expect(result.value).toBe('2026-03-');
  });

  it('keeps unpadded tokens unpadded', () => {
    const result = new LuxonDateInputAutocomplete('M/d/yyyy H:mm').process('7/5/2026 4:09', {
      now,
      commit: true,
    });

    expect(result.value).toBe('7/5/2026 4:09');
    expect(result.date?.toISO()).toBe(DateTime.fromISO('2026-07-05T04:09:00').toISO());
  });

  it('rejects impossible day and month combinations', () => {
    const result = new LuxonDateInputAutocomplete('dd-MM-yyyy').process('31-02', { now });
  ...
  it('validates leap years when the year is available', () => {
    const invalid = new LuxonDateInputAutocomplete('dd-MM-yyyy').process('29-02-2025', { now });
    const valid = new LuxonDateInputAutocomplete('dd-MM-yyyy').process('29-02-2024', { now });
  ...
  it('validates 12-hour values independently from 24-hour values', () => {
    const invalid = new LuxonDateInputAutocomplete('MM/dd/yyyy hh:mm a', 'en-US').process(
      '07/15/2026 13:59 PM',
      { commit: true, now },
    );
    const valid = new LuxonDateInputAutocomplete('MM/dd/yyyy hh:mm a', 'en-US').process(
      '07/15/2026 04:59 PM',
      { commit: true, now },
    );
  ...
  it('normalizes one-letter meridiem values on commit', () => {
    const result = new LuxonDateInputAutocomplete('M/d/yyyy h:mm a', 'en-US').process(
      '7/15/2026 4:59 p',
      { commit: true, now },
    );
  ...
  it('builds a completion suggestion from the current date and time', () => {
    const result = new LuxonDateInputAutocomplete('yyyy-MM-dd HH:mm:ss').process('2026-07-', {
      now,
    });
  ...
  describe('all requested formats', () => {
    it.each(SUPPORTED_FORMATS)('parses and round-trips $format', ({ format, locale }) => {
      const input = sampleDate.setLocale(locale).toFormat(format);
      const result = new LuxonDateInputAutocomplete(format, locale).process(input, {
        commit: true,
        locale,
        now,
      });
      ...
      expect(DateTime.fromFormat(result.value, format, { locale, setZone: true }).isValid).toBe(
        true,
      );
    });
    ...
    it.each(SUPPORTED_FORMATS.filter(({ smart }) => smart))(
      ...
      ({ format, locale }) => {
        ...
        const firstField =
          firstLiteralIndex === -1 ? fullValue : fullValue.slice(0, firstLiteralIndex);
        const firstLiteral = firstLiteralIndex === -1 ? '' : fullValue[firstLiteralIndex];
        const result = new LuxonDateInputAutocomplete(format, locale).process(firstField, {
          locale,
          now,
        });
  ...
  describe('pasted formatted values', () => {
    it('normalizes surrounding whitespace and adds an omitted Uhr suffix', () => {
      const result = new LuxonDateInputAutocomplete("dd.MM.yyyy HH:mm 'Uhr'").processPastedValue(
        '\u00a0 15.07.2026 16:59 \u00a0',
        { now },
      );

      expect(result.value).toBe('15.07.2026 16:59 Uhr');
      expect(result.date?.toISO()).toBe(DateTime.fromISO('2026-07-15T16:59:00').toISO());
    });

    it.each(['uhr', 'UHR', 'uHr'])('normalizes a case-insensitive %s suffix', (suffix) => {
      const result = new LuxonDateInputAutocomplete("dd.MM.yyyy HH:mm 'Uhr'").processPastedValue(
        `15.07.2026 16:59 ${suffix}`,
        { now },
      );
    ...
    it('ignores a single Uhr suffix when the configured format omits it', () => {
      const result = new LuxonDateInputAutocomplete('dd.MM.yyyy HH:mm').processPastedValue(
    ...
    it('does not silently ignore trailing pasted input', () => {
      const result = new LuxonDateInputAutocomplete('dd.MM.yyyy').processPastedValue(
        '15.07.2026 extra',
        { now },
      );
  ...
  describe('pasted epoch values', () => {
    it.each([
    ...
    ])('parses %s and renders the configured format', (_, epoch, expectedMilliseconds) => {
      const result = new LuxonDateInputAutocomplete("dd.MM.yyyy HH:mm:ss 'Uhr'").processPastedValue(
        epoch,
        { now },
      );
      ...
      expect(result.value).toBe(expected.toFormat("dd.MM.yyyy HH:mm:ss 'Uhr'"));
    });
  });
  ...
  describe('generic Luxon format support', () => {
    ...
    it.each([
      ...
      ['IANA zone', 'yyyy-MM-dd HH:mm z', '2026-07-15 16:59 Europe/Berlin', 'de-DE'],
    ])('parses a %s format', (_, format, value, locale) => {
      const result = new LuxonDateInputAutocomplete(format, locale).process(value, {
        commit: true,
        locale,
        now,
      });
  ...
  describe('additional regression coverage', () => {
    ...
    it('keeps compact dates distinct from epoch values', () => {
      const result = new LuxonDateInputAutocomplete('ddMMyyyy').processPastedValue('15072026', {
        now,
      });
    ...
    it('uses the seconds/milliseconds boundary consistently', () => {
      const autocomplete = new LuxonDateInputAutocomplete('yyyy-MM-dd HH:mm:ss');
    ...
    it.each([
      ...
      ['localized long weekday', 'yyyy-MM-dd EEEE', '2026-07-15 Mittwoch', 'de-DE'],
      ['quarter', "yyyy-'Q'q", '2026-Q3', 'en-US'],
      ['padded quarter', "yyyy-'Q'qq", '2026-Q03', 'en-US'],
      ['milliseconds', 'yyyy-MM-dd HH:mm:ss.SSS', '2026-07-15 16:59:12.125', 'en-US'],
      ['fractional seconds', 'yyyy-MM-dd HH:mm:ss.u', '2026-07-15 16:59:12.125', 'en-US'],
      ['two-digit fractional seconds', 'yyyy-MM-dd HH:mm:ss.uu', '2026-07-15 16:59:12.12', 'en-US'],
      ['one-digit fractional seconds', 'yyyy-MM-dd HH:mm:ss.uuu', '2026-07-15 16:59:12.1', 'en-US'],
    ...
    it.each([
      ...
      ['localized date-time with short zone', 'fff', '15. Juli 2026 um 16:59 +0200'],
    ])('keeps a %s value parser-compatible', (_, format, value) => {
      const result = new LuxonDateInputAutocomplete(format, 'de-DE').process(value, {
        commit: true,
        locale: 'de-DE',
        now,
      });
  ...
  it('provides default formats and descriptions', () => {
    expect(LuxonDateInputAutocomplete.getFormat({ dateOnly: true })).toBe('dd.MM.yyyy');
    expect(LuxonDateInputAutocomplete.getFormatDescription({ dateOnly: true })).toBe('TT.MM.JJJJ');
    expect(LuxonDateInputAutocomplete.getFormat({ showSeconds: true })).toBe(
      "dd.MM.yyyy HH:mm:ss 'Uhr'",
    );
    expect(LuxonDateInputAutocomplete.getFormatDescription({ showSeconds: true })).toBe(
  ...
  describe('empty input regression', () => {
    it.each([
      ...
      const result = new LuxonDateInputAutocomplete("dd.MM.yyyy HH:mm 'Uhr'", 'de-DE').process('', {
        now,
        ...processOptions,
      });
    ...
    it('keeps an empty pasted smart value empty', () => {
      const result = new LuxonDateInputAutocomplete('dd.MM.yyyy', 'de-DE').processPastedValue(
        '   ',
        { now },
      );
  ...
  describe('complete pasted values with leading non-numeric tokens', () => {
    it.each([
      ...
      ['literal and weekday', "'Date:' EEEE, dd.MM.yyyy", 'Date: Wednesday, 15.07.2026', 'en-US'],
    ])('parses a complete %s-leading pasted value', (_, format, value, locale) => {
      const result = new LuxonDateInputAutocomplete(format, locale).processPastedValue(value, {
        now,
        locale,
      });
