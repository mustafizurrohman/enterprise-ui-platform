...
describe('DatepickerComponent', () => {
  ...
  const dispatchPaste = (input: HTMLInputElement, value: string): void => {
    ...
    Object.defineProperty(event, 'clipboardData', {
      value: {
        getData: (type: string) => (type === 'text/plain' || type === 'text' ? value : ''),
      },
    });
  ...
  describe('testability', () => {
    it('should expose stable configurable test ids', async () => {
      expect(fixture.nativeElement.querySelector('[data-testid="datepicker"]')).toBeTruthy();
      expect(fixture.nativeElement.querySelector('[data-testid="datepicker-label"]')).toBeTruthy();
      expect(fixture.nativeElement.querySelector('[data-testid="datepicker-input"]')).toBeTruthy();
      expect(fixture.nativeElement.querySelector('[data-testid="datepicker-toggle"]')).toBeTruthy();
      expect(fixture.nativeElement.querySelector('[data-testid="datepicker-hint"]')).toBeTruthy();
      expect(
        fixture.nativeElement.querySelector('[data-testid="datepicker-input-status"]'),
      ).toBeTruthy();
      ...
      expect(document.querySelector('[data-testid="datepicker-dialog"]')).toBeTruthy();
      expect(document.querySelector('[data-testid="datepicker-calendar-grid"]')).toBeTruthy();
      expect(document.querySelector('datepicker-dialog')).toBeTruthy();
      expect(document.querySelector('datepicker-grid')).toBeTruthy();
      expect(document.querySelector('[data-testid^="datepicker-day-"]')).toBeTruthy();
      expect(document.querySelector('[data-testid="datepicker-hour-input"]')).toBeTruthy();
    });
    ...
    it('should have all documented test IDs from MDX', async () => {
      ...
      expect(fixture.nativeElement.querySelector('[data-testid="your-id"]')).toBeTruthy();
      expect(fixture.nativeElement.querySelector('[data-testid="your-id-input"]')).toBeTruthy();
      expect(fixture.nativeElement.querySelector('[data-testid="your-id-toggle"]')).toBeTruthy();

      (
        fixture.nativeElement.querySelector('[data-testid="your-id-toggle"]') as HTMLButtonElement
      ).click();
      fixture.detectChanges();
      await fixture.whenStable();

      expect(document.querySelector('[data-testid="your-id-dialog"]')).toBeTruthy();
    });
  });
  ...
  describe('dateOnly', () => {
    ...
    it('should hide time selection and use the date-only format', async () => {
      ...
      (fixture.nativeElement.querySelector('[data-testid$="toggle"]') as HTMLButtonElement).click();
      fixture.detectChanges();
      await fixture.whenStable();
    ...
    it('should describe the input-adjacent current-date action accurately', () => {
      ...
      expect(nowButton.getAttribute('aria-label')).toBe('Heutiges Datum auswählen');
    });
    ...
    it('should emit the selected date at the start of the day', () => {
      ...
      expect(onChangeSpy).toHaveBeenCalledWith(selectedDate.startOf('day').toJSDate());
    });
  });
  ...
  describe('calendar day arrangement', () => {
    ...
    it.each(monthStartCases)(
      ...
      (value, expectedColumn, expectedRow) => {
        ...
        expect(component.grid().every((week) => week.days.length === 7)).toBeTruthy();
      },
    );
  ...
  it('should keep one tabbable calendar day after month navigation', async () => {
    ...
    const tabbableDays = document.querySelectorAll('.datepicker-day[tabindex="0"]');
    expect(tabbableDays).toHaveLength(1);
    expect((tabbableDays[0] as HTMLElement).getAttribute('data-date')).toBe('2026-08-31');
  });
  ...
  it('should select now from the input-adjacent button', async () => {
    ...
    expect(selectedDate.toMillis()).toBeGreaterThanOrEqual(before.toMillis() - 1000);
  ...
  it('should update time', () => {
    ...
    expect(onChangeSpy).toHaveBeenLastCalledWith(component.selectedDate()?.toJSDate());
  });
  ...
  describe('Manual Input', () => {
    ...
    it.each([
    ...
    ])('should parse the custom Luxon format %s', (format, value, expectedIsoPrefix) => {
      ...
      expect(component.selectedDate()?.toISO()).toMatch(new RegExp(`^${expectedIsoPrefix}`));
      expect(input.value).toBe(component.selectedDate()?.setLocale('de-DE').toFormat(format));
    ...
    it('should fall back to the browser locale when locale is not provided', () => {
      ...
      expect(input.value).toBe(
        DateTime.fromISO('2026-07-15').setLocale(browserLocale).toFormat('dd MMMM yyyy'),
      );
      expect(component.daysOfWeek().map((day) => day.long)).toEqual(
    ...
    it('should parse a pasted datetime without requiring the Uhr suffix', () => {
      ...
      expect(onChangeSpy).toHaveBeenCalledWith(DateTime.fromISO('2026-07-15T16:59:11').toJSDate());
      expect(input.getAttribute('aria-invalid')).toBe('false');
    });
    ...
    ])('should parse pasted epoch %s and display the configured format', (_, epoch) => {
      fixture.componentRef.setInput('luxonDateFormat', "dd.MM.yyyy HH:mm:ss 'Uhr'");
      const input = fixture.nativeElement.querySelector(
        '[data-testid="datepicker-input"]',
      ) as HTMLInputElement;
      const expected = DateTime.fromMillis(epoch < 10_000_000_000 ? epoch * 1_000 : epoch);

      dispatchPaste(input, epoch.toString());
      fixture.detectChanges();

      expect(component.selectedDate()?.toMillis()).toBe(expected.toMillis());
      expect(input.value).toBe(expected.toFormat("dd.MM.yyyy HH:mm:ss 'Uhr'"));
      expect(input.getAttribute('aria-invalid')).toBe('false');
    });
    ...
    it('should expose an accessible status message after successful paste', () => {
      ...
      expect(status.textContent).toContain('Eingefügtes Datum und Uhrzeit übernommen.');
    });
  ...
  it('should have correct accessibility attributes and ID relationships', () => {
    const root = fixture.nativeElement.querySelector('[data-testid="datepicker"]') as HTMLElement;
    const wrapper = fixture.nativeElement.querySelector(
      '[data-testid="datepicker-input-wrapper"]',
    ) as HTMLElement;
    const label = fixture.nativeElement.querySelector('.datepicker-label') as HTMLLabelElement;
    const input = fixture.nativeElement.querySelector(
      '[data-testid="datepicker-input"]',
    ) as HTMLInputElement;
    const nowButton = fixture.nativeElement.querySelector(
    ...
    expect(input.getAttribute('aria-keyshortcuts')).toBe('ArrowDown Alt+ArrowDown Enter Escape');
    expect(input.getAttribute('aria-invalid')).toBe('false');
  ...
  it('should trap focus and have dialog attributes when calendar is open', async () => {
    ...
    const dialog = document.querySelector('.datepicker-calendar') as HTMLElement;
    const input = fixture.nativeElement.querySelector(
      '[data-testid="datepicker-input"]',
    ) as HTMLInputElement;
  ...
  it('should navigate calendar with keyboard', async () => {
    ...
    expect(
      (component as any).activeDate().hasSame(initialDate.plus({ days: 1 }), 'day'),
    ).toBeTruthy();
    ...
    expect(
      (component as any).activeDate().hasSame(initialDate.plus({ days: 8 }), 'day'),
    ).toBeTruthy();
  });

  describe('keyboard navigation', () => {
    const dispatchKey = (element: Element, key: string, shiftKey = false): void => {
      element.dispatchEvent(new KeyboardEvent('keydown', { key, shiftKey, bubbles: true }));
      fixture.detectChanges();
    };

    it('should NOT open the calendar from the input with Enter', async () => {
      const input = fixture.nativeElement.querySelector(
        '[data-testid="datepicker-input"]',
      ) as HTMLInputElement;
    ...
    it.each([
      ...
      'should open the calendar from the input with $key (Alt: $altKey)',
      async ({ key, altKey }) => {
        const input = fixture.nativeElement.querySelector(
          '[data-testid="datepicker-input"]',
        ) as HTMLInputElement;
    ...
    it.each(['Enter', ' '])('should select the focused date with %s', async (key) => {
      component.writeValue('2026-07-15T10:30:00');
      const onChangeSpy = vi.fn();
      component.registerOnChange(onChangeSpy);
  ...
  it('should allow typing time in time picker', async () => {
    ...
    const hourInput = document.querySelector('.datepicker-time-select') as HTMLInputElement;
  ...
  describe('disabled', () => {
    it("should disable the input and hide 'clear' and 'now' buttons when disabled is true", () => {
      ...
      const nowButton = fixture.nativeElement.querySelector('[data-testid$="now-input"]');
      const clearButton = fixture.nativeElement.querySelector('[data-testid$="clear"]');

      expect(input.disabled).toBeTruthy();
      expect(toggleButton.disabled).toBeTruthy();
      expect(toggleButton.classList.contains('datepicker-icon--disabled')).toBeTruthy();
  ...
  describe('format-derived mode', () => {
    ...
    it.each([
      ...
      ['dd.MM.yyyy', false],
    ])('should infer AM/PM selection from Luxon format %s', (format, expected) => {
      fixture.componentRef.setInput('luxonDateFormat', format);
      fixture.detectChanges();
    ...
    it('should render AM/PM directly after the minute wheel when seconds are omitted', async () => {
      ...
      expect(document.querySelector('[data-testid="datepicker-second-unit-control"]')).toBeNull();
      expect(
        minuteWheel.compareDocumentPosition(meridiemToggle) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
    });
    ...
    it('should render AM/PM after the seconds wheel only for a 12-hour format', async () => {
      ...
      expect(
        secondWheel.compareDocumentPosition(meridiemToggle) & Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
      ...
      expect(document.querySelector('[data-testid="datepicker-meridiem-toggle"]')).toBeNull();
      expect(component.selectedDate()?.hour).toBe(13);
      expect(
        (document.querySelector('[data-testid="datepicker-hour-input"]') as HTMLInputElement).value,
      ).toBe('13');
    });
    ...
    it('should switch meridiem when hour stepping crosses noon or midnight', async () => {
      ...
      (
        document.querySelector('[data-testid="datepicker-hour-increment"]') as HTMLButtonElement
      ).click();
      fixture.detectChanges();

      expect(component.selectedDate()?.hour).toBe(12);
      expect(
        (document.querySelector('[data-testid="datepicker-meridiem-pm"]') as HTMLInputElement)
          .checked,
      ).toBeTruthy();
      expect(
        (
      ...
      expect(component.selectedDate()?.hour).toBe(0);
      expect(
        (document.querySelector('[data-testid="datepicker-meridiem-am"]') as HTMLInputElement)
          .checked,
      ).toBeTruthy();
      expect(
        (
    ...
    it('should synchronize AM/PM changes with the selected DateTime and formatted input', async () => {
      ...
      expect(
        (
          document.querySelector('[data-testid="datepicker-hour-input"]') as HTMLInputElement
    ...
    it('should hide seconds wheel by default', async () => {
      ...
      expect(document.querySelector('#' + (component as any).ids().secondSelect)).toBeNull();
      expect(document.querySelectorAll('.datepicker-time-unit-control')).toHaveLength(2);
    });
    ...
    it('should show the seconds wheel when the format contains seconds', async () => {
      ...
      expect(document.querySelector('#' + (component as any).ids().secondSelect)).toBeTruthy();
      expect(document.querySelectorAll('.datepicker-time-unit-control')).toHaveLength(3);
    });
  ...
  describe('today input', () => {
    ...
    it('should reflect the provided today input in the template classes', async () => {
      ...
      const todayButton = document.querySelector(`.datepicker-day[data-date="2026-07-20"]`);
  ...
  describe('additional verification for time and copy-paste', () => {
    ...
    it('should display the invalid character in the input before showing the error', async () => {
      ...
      const error = fixture.nativeElement.querySelector('[data-testid="datepicker-error"]');
      expect(error).toBeTruthy();
    });
  ...
  describe('showQuickTimeControls', () => {
    ...
    it('should hide quick-time controls by default', async () => {
      ...
      const adjustments = document.querySelector('[data-testid="datepicker-time-adjustments"]');
