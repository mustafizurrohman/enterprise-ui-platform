...
describe('TimeUnitControlComponent', () => {
  ...
  const getValueContainer = (): HTMLElement =>
    fixture.nativeElement.querySelector('[data-testid="datepicker-hour-value"]') as HTMLElement;
  ...
  it('should render the increment button above the decrement button', () => {
    ...
    const buttons = Array.from(buttonStack.querySelectorAll<HTMLButtonElement>('button'));
  ...
  it('should repeat increment while pressed and accelerate from 70 ms to 50 ms', () => {
    ...
    const acceleratingDelays = [300, 294, 288, 282, 276, 270, 264, 258, 252, 246, 240];

    for (const delay of acceleratingDelays) {
      vi.advanceTimersByTime(delay - 1);
      expect(offsetChangeSpy).toHaveBeenCalledTimes(acceleratingDelays.indexOf(delay) + 1);

      vi.advanceTimersByTime(1);
      expect(offsetChangeSpy).toHaveBeenCalledTimes(acceleratingDelays.indexOf(delay) + 2);
    }
  ...
  it.each([
    ...
    ['Home', 'value', 0],
    ['End', 'value', 23],
  ])('should handle %s keyboard input', (key: string, type: string, expectedValue: number) => {
    const valueChangeSpy = vi.fn();
    const offsetChangeSpy = vi.fn();
    component.valueChange.subscribe(valueChangeSpy);
    component.offsetChange.subscribe(offsetChangeSpy);
  ...
  it('should set the increment CSS animation on an increment', () => {
    ...
    expect(getValueContainer().getAttribute('data-animation')).toBe('increment-a');
    expect(getValueContainer().classList.contains('datepicker-time-value--rapid')).toBeFalsy();
  });
  ...
  it('should set the decrement CSS animation on a decrement', () => {
    ...
    expect(getValueContainer().getAttribute('data-animation')).toBe('decrement-a');
  });
  ...
  it('should alternate CSS animation names so rapid changes restart', () => {
    ...
    expect(getValueContainer().getAttribute('data-animation')).toBe('increment-b');
  });
  ...
  it('should use the rapid CSS duration class for fast button presses', () => {
    ...
    expect(getValueContainer().classList.contains('datepicker-time-value--rapid')).toBeTruthy();
  });
